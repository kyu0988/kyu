...
retryButton.addEventListener('click', resetOmikuji);
stick.addEventListener('click', drawFortune);
...
function displayResult(level) {
    ...
    // 各運勢
    document.getElementById('generalFortune').textContent = getRandomItem(data.general);
    document.getElementById('loveFortune').textContent = getRandomItem(data.love);
    document.getElementById('moneyFortune').textContent = getRandomItem(data.money);
    document.getElementById('healthFortune').textContent = getRandomItem(data.health);

    // ラッキーカラーとナンバー
    const luckyColor = getRandomItem(data.colors);
    document.getElementById('luckyColor').innerHTML =
        `<span style="display: inline-block; width: 20px; height: 20px; background: ${getColorCode(luckyColor)}; border-radius: 50%; margin-right: 5px; vertical-align: middle; border: 2px solid #ddd;"></span>${luckyColor}`;

    document.getElementById('luckyNumber').textContent = Math.floor(Math.random() * 99) + 1;

    // アドバイス
    document.getElementById('adviceText').textContent = getRandomItem(data.advice);
...
function updateBackgroundColor(level) {
    ...
    body.style.background = gradients[level];
